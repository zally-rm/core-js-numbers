...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const denominator1 = Math.sqrt(x1 * x1 + y1 * y1);
  const denominator2 = Math.sqrt(x2 * x2 + y2 * y2);
  const bigParentheses = product / (denominator1 * denominator2);
  const radians = Math.acos(bigParentheses);
  return radians;
}
...
function roundToPowerOfTen(num, pow) {
  const power = 10 ** pow;
  const roundedNum = Math.round(num / power) * power;
  return roundedNum;
}
...
function getSumOfDigits(num) {
  ...
  const strArray = stringNum.split('');
  let sum = 0;
  for (let i = 0; i < strArray.length; i += 1) {
    sum += parseInt(strArray[i], 10);
  }
  return sum;
}
...
function isPowerOfTwo(num) {
  const validPow = Math.log2(num) % 1;
  return validPow === 0;
}
...
function numberToStringInBase(number, base) {
  const strRepresent = number.toString(base);
  return strRepresent;
}
...
function toExponential(number, fractionDigits) {
  const representFraction = number.toExponential(fractionDigits);
  return representFraction;
}
...
function toFixed(number, fractionDigits) {
  const fixedNtn = number.toFixed(fractionDigits);
  return fixedNtn;
}
...
function toPrecision(number, precision) {
  const strPosition = number.toPrecision(precision);
  return strPosition;
}
...
function getFloatOnString(str) {
  const strFloat = Number.parseFloat(str);
  return strFloat;
}
...
function getIntegerOnString(str, base) {
  const strBase = Number.parseInt(str, base);
  return strBase;
}
...
function getCountOfOddNumbers(number) {
  const sumCount = Math.floor((Math.abs(number) + 1) / 2);
  return sumCount;
}
